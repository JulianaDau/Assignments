...
my_histo <- function(x=NULL,data_type="cont") {
  if(is.null(x)) x=runif(1000) # if user didn`t give data use 1000 random `
  df <- data.frame(x=x) 
  #needs to be inside a dataframe
  #bellow multiple if statements 
  # if data are continuous bounded (0,1), adjust bins for histogram   min(df$x) used to show that above 0
...
my_histo(data_type="disc",x=rpois(1000,lambda=0.2))
my_histo(data_type="cont",x=runif(1000))
my_histo(data_type="cont",x=rnorm(n=1000,mean=0,sd=1)) #simmulation of a normal distribuition
############################
# Function: my_pdf
# Purpose: creates a ggplot probability density function
# Requires: ggplot
...
my_pdf <- function(x=NULL,y=NULL,data_type="cont") {
  ...
  # for continuous distributions, 
  # plot the line for the pdf
  #first x and first y is what the program will look for
  if(data_type=="cont") {
    p1 <- ggplot(data=df) +
      aes(x=x,y=y) +
      geom_line() +
...
nbi_ran <- rnbinom(n=1000,size=0.1,mu=5) # size parameter will control the variance 
my_histo(x=nbi_ran,data_type="disc")

nbi_ran_large <- rnbinom(n=1000,size=10,mu=5)
my_histo(x=nbi_ran_large,data_type="disc")
...
#can be a introduction to basean statictics

# shape1 = 1, shape2 = 1 = "no data" , sample size of zero. Expectation, prior distribution between 0 and 1
...
#fitting using data
library(MASS)
data <- c(100,100,104,99)  # 4 data value
z <- fitdistr(data,"normal") # the user determine the type so will analyse if it was normal (but this a estimate if it is normal)
print(z)
...
g_density <- dnorm(x=x,mean=z$estimate["mean"],sd=z$estimate["sd"])
qplot(x,g_density,geom="line") + annotate(geom="point",x=frog_data,y=0.001)   
#although fitted for normal doesn`t seem like the ideal `
